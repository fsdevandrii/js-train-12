...
function sortByValues(dictionary) {
  ...
  const sortedArray = Array.from(dictionary).sort((a, b) => b[1] - a[1]);
...
function resetDictionary(dictionary, maxSize) {
  ...
  // Перевіряємо, чи кількість елементів у словнику більше, ніж максимально допустима
  if (dictionary.size > maxSize) {
    // Якщо умова виконується, очищуємо словник за допомогою методу clear
    dictionary.clear();
    // Повертаємо true, оскільки словник був очищений
    return true;
  } else {
    // Якщо умова не виконується, повертаємо false, оскільки словник не був очищений
    return false;
  }
}
...
function convertDictionaryToSet(dictionary) {
  ...
  // Створюємо нову множину для збереження результатів
  let resultSet = new Set();

  // Перебираємо ключі словника
  for (let key of dictionary.keys()) {
    // Якщо ключ є рядковим значенням, перетворюємо його в верхній регістр і додаємо до множини
    if (typeof key === "string") {
      resultSet.add(key.toUpperCase());
    }
    // Якщо ключ є числовим значенням, збільшуємо його на 1 і додаємо до множини
